...
{
	os << "cpx_" << g_FunctionNames[static_cast<size_t>(name)] << '(';
	if (param)
		os << (*param);
	os << ')';
}
...
void FunctionParser::Operator::Print(std::ostream& os) const
{
	...
	os << "cpx_" << opNames[static_cast<size_t>(name)] << '(';
	if (params[0])
		os << (*params[0]);
	os << ',';
	if (params[1])
	...
	if (IsLetter(func[offset]))
	{
		...
		else
		{
			if (name == "i")
				...
				funcElem = std::make_unique<Constant>(std::complex<double>(M_PI, 0.0));
			else if (name == "e")
				funcElem = std::make_unique<Constant>(std::complex<double>(M_E, 0.0));
			else if (name == "c")
			{
